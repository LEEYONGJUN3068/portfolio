    ...
    shapeRefs.current.forEach(shape => {
      if (shape) { // 도형 요소가 null이 아닐 때만 애니메이션을 적용합니다.
        gsap.to(shape, {
          ...
          x: gsap.utils.random(-60, 60), // x축으로 -30 ~ 30 단위 무작위 이동
          y: gsap.utils.random(-60, 60), // y축으로 -30 ~ 30 단위 무작위 이동
          rotation: gsap.utils.random(0, 360), // 0도 ~ 360도 무작위 회전
          opacity: gsap.utils.random(0.05, 0.2), // 0.05 ~ 0.2 무작위 투명도 (매우 은은하게)
          scale: gsap.utils.random(0.1, 0.1), // 크기를 0.8배 ~ 1.2배 무작위로 변경
          duration: gsap.utils.random(5, 10), // 한 애니메이션 사이클이 10초 ~ 20초 동안 지속 (매우 느리게)
          repeat: -1, // 무한 반복
          yoyo: true, // 애니메이션이 끝난 후 역방향으로 돌아옴 (왕복 움직임)
