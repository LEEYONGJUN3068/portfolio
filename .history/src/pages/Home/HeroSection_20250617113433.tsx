...
const HeroSection: React.FC = () => {
  ...
  const dotRefs = useRef<(SVGCircleElement | null)[]>(Array(numDots).fill(null));
  ...
  const dots = Array.from({ length: numDots }).map((_, i) => (
    <circle
      ...
      cx={gsap.utils.random(0, 100)} // SVG viewBox 기준 초기 X 위치 (0~100 사이)
      cy={gsap.utils.random(0, 100)} // SVG viewBox 기준 초기 Y 위치 (0~100 사이)
      r={gsap.utils.random(0.5, 1.5)} // 점의 반지름 (0.5 ~ 1.5 사이)
      fill="#4A5568" // 점의 색상 (짙은 회색 계열)
      opacity={0.3} // 초기 투명도 (은은하게)
      ref={el => (dotRefs.current[i] = el)} // useRef 배열에 요소 참조 저장
    />
  ));
